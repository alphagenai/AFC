...
MONTHLY_CUMULATIVE_PERCENT_PIVOT = create_percent_sdf(monthly_sdf_pivot, cumulative=True, cohort='dec_17')


DAILY_SDF = daily_sdf = df.groupby(['ContractId', pd.Grouper(freq='D', level=1)])['AmountPaid'].sum()
DAILY_SDF_PIVOT = daily_sdf_pivot = daily_sdf['AmountPaid'].unstack(0).fillna(0)

DAILY_CUMULATIVE_PERCENT_SDF = daily_cumulative_percent_sdf_pivot = create_percent_sdf(daily_sdf_pivot, cumulative=True, cohort='dec_17')
